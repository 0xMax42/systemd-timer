...
Deno.test('loadLocale lade Deutsche Übersetzung', async () => {
    await loadLocale('de');
    assertEquals(t('error_write_units'), 'Fehler beim Schreiben der Units:');
    assertEquals(
...
Deno.test(
    't gibt den Schlüssel zurück, wenn dieser nicht gefunden wird',
    () => {
...
Deno.test(
    'getCurrentLanguage nutz die `SYSTEMD_TIMER_LANG`, wenn gesetzt',
    () => {
